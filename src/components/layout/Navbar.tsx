...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleMenuEnter = (menu: 'products' | 'services' | 'solutions') => {
          ...
          <nav className="hidden lg:flex items-center gap-2 font-medium text-sm text-[#1F2933]">
            ...
            >
              <button
                onClick={() => {
                  onNavigateProducts();
                  if (activeMenu === 'products') handleImmediateClose();
                  else handleMenuEnter('products');
                }}
            ...
            >
              <button
                onClick={() => {
                  onNavigateServices();
                  if (activeMenu === 'services') handleImmediateClose();
                  else handleMenuEnter('services');
                }}
                className="relative flex items-center gap-1.5 px-3 py-2 transition-colors duration-150 cursor-pointer group select-none"
              >
                <span className={`transition-colors duration-150 ${
